...
func testUserToManyCreditCards(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, creditCardDBTypes, false, creditCardColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, creditCardDBTypes, false, creditCardColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.CreditCards().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadCreditCards(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CreditCards); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.CreditCards = nil
	if err = a.L.LoadCreditCards(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CreditCards); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpCreditCards(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e CreditCard

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*CreditCard{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, creditCardDBTypes, false, strmangle.SetComplement(creditCardPrimaryKeyColumns, creditCardColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*CreditCard{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddCreditCards(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.CreditCards[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.CreditCards[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.CreditCards().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Name`: `text`, `Email`: `text`, `Password`: `text`, `RefreshToken`: `text`}
	_           = bytes.MinRead
)

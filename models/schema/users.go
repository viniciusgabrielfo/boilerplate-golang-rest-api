...
// CreditCards retrieves all the credit_card's CreditCards with an executor.
func (o *User) CreditCards(mods ...qm.QueryMod) creditCardQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"credit_cards\".\"user_id\"=?", o.ID),
	)

	query := CreditCards(queryMods...)
	queries.SetFrom(query.Query, "\"credit_cards\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"credit_cards\".*"})
	}

	return query
}

// LoadCreditCards allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadCreditCards(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`credit_cards`), qm.WhereIn(`credit_cards.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load credit_cards")
	}

	var resultSlice []*CreditCard
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice credit_cards")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on credit_cards")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for credit_cards")
	}
	...
	if singular {
		object.R.CreditCards = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &creditCardR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.CreditCards = append(local.R.CreditCards, foreign)
				if foreign.R == nil {
					foreign.R = &creditCardR{}
				}
...
// AddCreditCards adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.CreditCards.
// Sets related.R.User appropriately.
func (o *User) AddCreditCards(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*CreditCard) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.CreditCards = append(o.R.CreditCards, related...)
	}
